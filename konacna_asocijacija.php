<?php
	...
	if(isset($_POST['nick']))
	{
		...
		if(proveri_resenje(pojam($podaci[1]),pojam($podaci[2]),pojam($podaci[3]),pojam($podaci[4]),$as,'gr-rk'))
		{
			$bodovi=0;
			$bodovi+=20+10+(16-intval($podaci[21]));
			addExp($nick,$bodovi+floor(intval(get_reputacija($nick))/100));
			echo "ok"."*".pojam($podaci[5])."*".pojam($podaci[6])."*".pojam($podaci[7])."*".pojam($podaci[8])."*".pojam($podaci[9])."*".pojam($podaci[10])."*".pojam($podaci[11])."*".pojam($podaci[12])."*".pojam($podaci[13])."*".pojam($podaci[14])."*".pojam($podaci[15])."*".pojam($podaci[16])."*".pojam($podaci[17])."*".pojam($podaci[18])."*".pojam($podaci[19])."*".pojam($podaci[20])."*".pojam($podaci[1])."*".pojam($podaci[2])."*".pojam($podaci[3])."*".pojam($podaci[4])."*".pojam($podaci[0])."*".strval($bodovi);
